  ...
  <div class="flex min-h-screen w-full flex-col bg-gradient-to-br from-gray-950 via-indigo-950 to-purple-950 text-white relative overflow-hidden">
    ...
    <aside class="fixed inset-y-0 left-0 z-20 w-64 flex-col border-r border-indigo-700/50 bg-gray-900/30 backdrop-blur-lg hidden sm:flex">
      <nav class="flex flex-col gap-6 px-4 py-6 text-lg font-medium">
        <NuxtLink to="/" class="group flex items-center justify-center h-12 w-full rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-white transition-all duration-300 hover:from-blue-500 hover:to-purple-500 shadow-glow">
          <span class="ml-2 text-sm font-semibold animate-flicker">NeuralNet AI</span>
        </NuxtLink>
        <NuxtLink
          to="/chat"
          class="flex items-center gap-4 px-2.5 text-gray-300 hover:text-white hover:bg-indigo-800/50 rounded-lg py-2 transition-all duration-200 shadow-glow"
          :class="{ 'text-white bg-indigo-700/50': $route.path === '/chat' }"
        >
          <Package class="h-5 w-5" /> Chats
        </NuxtLink>
        <NuxtLink
          v-if="['coder', 'admin'].includes(userStore.role)"
          to="/generation"
          class="flex items-center gap-4 px-2.5 text-gray-300 hover:text-white hover:bg-indigo-800/50 rounded-lg py-2 transition-all duration-200 shadow-glow"
          :class="{ 'text-white bg-indigo-700/50': $route.path === '/generation' }"
        >
        ...
        <NuxtLink
          ...
          to="/admin-dashboard"
          class="flex items-center gap-4 px-2.5 text-gray-300 hover:text-white hover:bg-indigo-800/50 rounded-lg py-2 transition-all duration-200 shadow-glow"
          :class="{ 'text-white bg-indigo-700/50': $route.path === '/admin-dashboard' }"
        >
          <LineChart class="h-5 w-5" /> Admin Dashboard
        </NuxtLink>
      </nav>
    </aside>
    <!-- Main Layout -->
    <div class="flex flex-col sm:gap-4 sm:py-4 sm:pl-64">
      <header class="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-indigo-700/50 bg-gray-900/30 backdrop-blur-lg px-4 sm:px-6">
          ...
          <SheetContent side="left" class="sm:max-w-xs bg-gray-900/30 text-white border-indigo-700/50 backdrop-blur-lg">
            <nav class="grid gap-6 text-lg font-medium">
              ...
              <NuxtLink
                ...
                to="/api-keys"
                class="flex items-center gap-4 px-2.5 text-gray-300 hover:text-white hover:bg-indigo-800/50 rounded-lg py-2 transition-all duration-200 shadow-glow"
                :class="{ 'text-white bg-indigo-700/50': $route.path === '/api-keys' }"
              >
                <Key class="h-5 w-5" /> API Keys
              </NuxtLink>
              <NuxtLink
        ...
        <Breadcrumb class="hidden md:flex text-gray-300 animate-glow-text">
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink as-child><NuxtLink to="/" class="hover:text-white transition-colors">Dashboard</NuxtLink></BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" class="bg-gray-800/30 text-white border border-indigo-700/50 backdrop-blur-lg">
            <DropdownMenuLabel>Role: {{ userStore.role }}</DropdownMenuLabel>
            ...
            <!-- Для отладки: временная кнопка для установки роли -->
            <DropdownMenuItem @click="userStore.setRole('admin')" class="hover:bg-indigo-700/50">Set Role to Admin (Debug)</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </header>
...
import { CircleUser, File, Key, LineChart, ListFilter, MoreHorizontal, Package, PanelLeft, PlusCircle, Search } from 'lucide-vue-next'
...
function authCheck(): string {
  ...
  if (!token) {
    console.error('authCheck: Токен отсутствует')
...
async function fetchWithToken<T>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  console.log('fetchWithToken: Запрос к', url, 'с токеном:', token)
...
const { data: fetchedChats, refresh } = await useAsyncData('chats', async () => {
  try {
    ...
    console.log('fetchChats: Получены чаты:', res)
    return res.map(chat => ({
      id: Number(chat.id),
    ...
    console.error('fetchChats: Ошибка при загрузке чатов:', err)
    return []
  }
}, { server: false })
...
async function goToChat(chatId: number) {
  try {
    ...
    console.error('goToChat: Ошибка перехода в чат:', err)
...
async function createChat() {
  try {
    ...
    console.error('createChat: Ошибка при создании чата:', err)
...
async function saveChat() {
  ...
  try {
    ...
    console.error('saveChat: Ошибка при сохранении:', err)
...
async function deleteChat(chatId: number) {
  ...
  try {
    ...
    console.error('deleteChat: Ошибка при удалении чата:', err)
...
async function sendMessage(chatId: number | null, message: string) {
  ...
  try {
    ...
    console.error('sendMessage: Ошибка при отправке сообщения:', err)
